Shader "Unlit/ShieldShader2"
{
	Properties
	{
	   ...
	   _T("Float Time", Range(0.1, 30)) = 5
	   _X("Float Time", Range(0.1, 30)) = 5
	}
		SubShader
	   {
		  Tags {
		  ...
		  Pass
		  {
			 ...
			 float triWave(float t, float offset, float yOffset) {
				 return saturate(abs(frac(offset + t) * _X / 3 - 1) + yOffset);
			 }

			 fixed4 texColor(v2f i, float rim) {
				 fixed4 mainTex = tex2D(_MainTex, i.uv);
				 mainTex.r *= triWave(_Time.x * _T, abs(i.objectPos.y) * 2, -0.7) * 6;
				 mainTex.g *= saturate(rim) * (sin(_Time.z + mainTex.b * _T) + 1);
